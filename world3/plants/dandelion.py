...
class Dandelion(Grass):
    def __init__(self,  _world=None, _x=None, _y=None, _age=0, _init=0, _power=0):
        super(Dandelion, self).__init__(_world, _x, _y, _age, _init, _power)
    ...
    def Divide(self, _chances):
        if len(_chances) == 0:
            ...
            if _chances[0] == Direction.UP:
                self.world.ToAdd(Dandelion(self.world, self.x-1, self.y))
            elif _chances[0] == Direction.DOWN:
                self.world.ToAdd(Dandelion(self.world, self.x+1, self.y))
            elif _chances[0] == Direction.RIGHT:
                self.world.ToAdd(Dandelion(self.world, self.x, self.y + 1))
